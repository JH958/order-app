...
export async function seedIfEmpty() {
  try {
    ...
    if (count > 0) {
      console.log(`✅ 메뉴 데이터가 이미 존재합니다 (${count}개)`);
      return;
    }
    ...
    for (const menu of menus) {
      try {
        ...
        console.error(`⚠️  메뉴 "${menu.name}" 삽입 중 오류:`, error.message);
